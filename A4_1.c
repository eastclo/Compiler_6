...
//set delcarator element type and kind
...
BOOLEAN isNotSameType(A_TYPE *t1, A_TYPE *t2){
	if(isPointerOrArrayType(t1) || isPointerOrArrayType(t2))
		return isNotSameType(t1->element_type, t1->element_type);
	else
		return (t1 != t2);
}
...
void syntax_error(int i, char *s){
	...
	switch(i){
		...
		case 12: printf("rdeclaration if identifier %s", s); break;
		case 13: printf("undefined identifier %s", s); break;
		case 14: printf("illegal tyep specifier in formal parameter"); break;
		case 20: printf("illegal storage class in type specifier"); break;
